    	 ...
    	 if(/map\/getmasssendmsg\?__biz/i.test(req.url)){// 当链接地址为公众号历史消息页面时(第一种页面形式)
             if(serverResData.toString() !== ""){
                 try {// 防止报错退出程序
                     ...
                     http.get('http://192.168.1.117/crazy_blog/getWxHis', function(res) {// 这个地址是自己服务器上的一个程序，目的是为了获取到下一个链接地址，将地址放在一个js脚本中，将页面自动跳转到下一页。后文将介绍getWxHis.php的原理。
             ...
             try {
                 ...
                 var ret = reg.exec(serverResData.toString());// 转换变量为string
                  HttpPost(ret[1],req.url,"/crazy_blog/getMsgJson",serverResData,callback);// 这个函数是后文定义的，将匹配到的历史消息json发送到自己的服务器
                 var content = serverResData.toString();
                   weixin = serverResData;
                // var http = require('http');
             ...
             try {
                 ...
                 if (json.general_msg_list != []) {
                    ...
                    HttpPost(json.general_msg_list,req.url,"/crazy_blog/getMsgJson",serverResData,callback);// 这个函数和上面的一样是后文定义的，将第二页历史消息的json发送到自己的服务器
                     console.log(pok+"************公众号历史消息(翻页后的json)**************"+req.url);
             ...
             try {
                 var http = require('http');
                 http.get('http://192.168.1.117/crazy_blog/getWxPost', function(res) {// 这个地址是自己服务器上的另一个程序，目的是为了获取到下一个链接地址，将地址放在一个js脚本中，将页面自动跳转到下一页。后文将介绍getWxPost.php的原理。
                     res.on('data', function(chunk){
...
function HttpPost(str,url,path,serverResData,callback) {// 将json发送到服务器，str为json内容，url为历史消息页面地址，path是接收程序的路径和文件名
    var http = require('http');
    var data = {
