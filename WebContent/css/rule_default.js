	    ...
	    *beforeSendResponse(requestDetail, responseDetail) {
	        try {
	            ...
	            if(/mp\/profile_ext\?action=home/i.test(link)){
	                ...
	                // 数据上传到服务器
	                serverPost(biz, account, '/crazy_blog/getMsgJson')
	            ...
	            if(/mp\/profile_ext\?action=getmsg/i.test(link)){
	                ...
	                serverPost(biz, articles, '/crazy_blog/getMsgJson');
	                return null;
	            }
	            ...
	            if (/\/s\?__biz/.test(link) || /mp\/appmsg\/show/.test(link)) {
	                ...
	                let bb = async function() {
	                    return await serverPost(article.biz, article, "/crazy_blog/getMsgJson").then((nextLink) => {
	            ...
	            if (/\/mp\/appmsg_comment/.test(link)){
	                ...
	                if (comments.length > 0) {
	                    serverPost(comments[0].biz, comments, '/crazy_blog/getMsgJson');
	                }
	                return null;
	            }
	            ...
	            if (link.indexOf('getappmsgext') > -1) {
	                ...
	                serverPost(article.biz, article, '/crazy_blog/getMsgJson');
	                return null;
	            }
